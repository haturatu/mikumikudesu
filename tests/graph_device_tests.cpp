...
using dayo::graphics::PhysicalResourceRequirements;
...
int main() {
    ...
    // ---- native graphics descriptors retain MRT/DSV pipeline state ----
    {
        ...
        pipeline.colorFormats = {dayo::graphics::PixelFormat::rgba16Float, dayo::graphics::PixelFormat::rgba16Float,
        ...
        ok &= check(pipeline.colorFormats.size() == 3 && pipeline.depthFormat.has_value() &&
                        pipeline.rasterizer.cullMode == dayo::graphics::CullModeEx::front &&
                        !pipeline.depthStencil.depthWrite && !pipeline.depthOnly &&
                        pipeline.blendAttachments.front().enabled,
    ...
    // ---- alias判定: transient may alias, persistent history may not ----
    {
        ...
        PhysicalResourceRequirements physicalA{256, 256, 0x3U, false, true};
        PhysicalResourceRequirements physicalB{512, 128, 0x2U, false, true};
        ok &= check(dayo::graphics::canPhysicallyAlias(ResourceLifetime::transient, ResourceLifetime::transient,
                                                       physicalA, physicalB),
                    "backend alias gate accepts compatible memory requirements");
        physicalB.memoryTypeBits = 0x4U;
        ok &= check(!dayo::graphics::canPhysicallyAlias(ResourceLifetime::transient, ResourceLifetime::transient,
                                                        physicalA, physicalB),
                    "backend alias gate rejects disjoint memory types");
        physicalB.memoryTypeBits = 0x2U;
        physicalB.requiresDedicatedAllocation = true;
        ok &= check(!dayo::graphics::canPhysicallyAlias(ResourceLifetime::transient, ResourceLifetime::transient,
                                                        physicalA, physicalB),
                    "backend alias gate rejects dedicated allocations");
        ...
        TextureResourceDesc invalidCube = makeTransientDesc();
        invalidCube.dimension = TextureDimension::cube;
        invalidCube.arrayLayers = 1;
        invalidCube.extent.height = 32;
        ok &= check(!dayo::graphics::isValidTextureDesc(invalidCube), "cube validation requires square faces");
    }
    ...
    // ---- aliasing with lifetime overlap: disjoint transient may alias ----
    {
        ...
        RenderGraph mixed;
        const auto mixedTexture = mixed.createResource("texture", makeTransientDesc());
        const auto mixedBuffer = mixed.createBufferResource("buffer", transientBuffer);
        ok &= check(!mixed.mayAliasByLifetime(mixedTexture, mixedBuffer),
                    "texture and buffer are not lifetime-only alias pairs");
        const auto unused = mixed.createResource("unused", makeTransientDesc());
    ...
    // ---- VMA allocation hooks ----
    {
        ...
        TextureResourceDesc cube = makeTransientDesc();
        cube.dimension = TextureDimension::cube;
        cube.arrayLayers = 2;
        const auto cubeBytes = dayo::graphics::estimateTextureBytes(cube);
        ok &= check(cubeBytes == 12U * dayo::graphics::estimateTextureBytes(makeTransientDesc()),
                    "cube allocation counts six faces per cube");
        TextureResourceDesc overflowing = makeTransientDesc();
        overflowing.extent = {std::numeric_limits<std::uint32_t>::max(), std::numeric_limits<std::uint32_t>::max(), 1};
        bool overflowThrew = false;
        try {
            static_cast<void>(dayo::graphics::estimateTextureBytes(overflowing));
        } catch (const std::overflow_error&) {
            overflowThrew = true;
        }
        ok &= check(overflowThrew, "VMA size estimate rejects arithmetic overflow");
    ...
    // ---- Device拡張 (part 2): copy paths + layouts fail explicitly ----
    {
        ...
        bool samplerThrew = false;
        try {
            static_cast<void>(device.createSamplerEx());
        } catch (const std::logic_error&) {
            samplerThrew = true;
        }
