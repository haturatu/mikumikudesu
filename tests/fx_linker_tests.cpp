...
int main() {
    ...
    // Linker produces FxProgram from parsed EffectGraph documents (§1).
    {
        dayo::core::EffectGraph graph;
        graph.category = "test-effect";
        EffectPass raster;
        raster.name = "main";
        raster.type = EffectPassType::rasterizer;
        raster.vertexShader = "vs";
        raster.pixelShader = "ps";
        raster.conditions = {"on frame"};
        EffectPass cs;
        cs.name = "sim";
        cs.type = EffectPassType::compute;
        cs.computeShader = "cs_main";
        graph.passes = {raster, cs};
        const auto program = linkFxProgram(graph, "render");
        ok &= check(program.passes.size() == 2, "linker keeps pass count");
        ok &= check(std::holds_alternative<FxRasterOp>(program.passes[0].op), "linker types raster pass");
        ok &= check(std::holds_alternative<FxComputeOp>(program.passes[1].op), "linker types compute pass");
        ok &= check(program.category == FxCategory::render, "linker records category");
        ok &= check(program.passes[0].eventMask == kFxEventFrame, "linker folds pass conditions to mask");
        ok &= check(program.scheduler.size() == 2, "linker wires scheduler");
        const auto onFrame = program.scheduler.activePasses(kFxEventFrame);
        ok &= check(onFrame.size() == 2, "frame serves conditional + unconditional passes");
        const auto onLoad = program.scheduler.activePasses(kFxEventLoad);
        ok &= check(onLoad.size() == 1 && onLoad[0] == 1, "load serves only unconditional pass");
        MaterialTemplate mat;
        mat.name = "mat";
        mat.resources = {{.id = "albedo", .shared = true}};
        const auto withMat = linkFxProgram(graph, "render", mat, nullptr);
        ok &= check(withMat.material.layout.slotForCanonical("shared:albedo") == "_R0", "linker plans material slots");
    }

    // Lifecycle mapping: load/start/resize/frame points hit distinct masks.
    {
        ok &= check(fxEventForLifecyclePoint(FxLifecyclePoint::load) == kFxEventLoad, "lifecycle load");
        ok &= check(fxEventForLifecyclePoint(FxLifecyclePoint::start) == kFxEventStart, "lifecycle start");
        ok &= check(fxEventForLifecyclePoint(FxLifecyclePoint::resize) == kFxEventResize, "lifecycle resize");
        ok &= check(fxEventForLifecyclePoint(FxLifecyclePoint::frame) == kFxEventFrame, "lifecycle frame");
        ok &= check(std::string(toString(FxLifecyclePoint::resize)) == "resize", "lifecycle to string");
        const FxFrameContext context;
        ok &= check(context.cloneCount == 1, "frame context defaults to one clone");
        ok &= check(context.frameIndex == 0 && context.sampleIndex == 0, "frame context starts at zero");
        FxInstance instance;
        ok &= check(instance.activePasses(kFxEventFrame).empty(), "null program plans nothing");
    }

    // Schedule-time raster target expansion (never in the executor).
    {
        const auto all = expandRasterModelTargets(RasterModelTarget::all, 3, 1);
        ok &= check(all.size() == 3 && all[0] == 0 && all[2] == 2, "target all fans out");
        const auto self = expandRasterModelTargets(RasterModelTarget::self, 3, 1);
        ok &= check(self.size() == 1 && self[0] == 1, "target self stays concrete");
        const auto other = expandRasterModelTargets(RasterModelTarget::other, 3, 1);
        ok &= check(other.size() == 2 && other[0] == 0 && other[1] == 2, "target other excludes self");
        ok &= check(expandRasterModelTargets(RasterModelTarget::buffer, 3, 1).empty(), "target buffer fans out empty");
        ok &= check(expandRasterModelTargets(RasterModelTarget::self, 0, 0).empty(), "empty scene expands empty");
        ok &= check(expandRasterModelTargets(RasterModelTarget::self, 2, 9).empty(), "dangling self expands empty");
    }
