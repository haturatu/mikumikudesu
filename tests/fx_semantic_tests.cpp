...
int main() {
    ...
    // --- dependencies ---
    ok &= check(fxDependencies(parseFxExpr("1+2")) == FxExprDependency::StaticDependency, "dep static");
    ok &= check(any(fxDependencies(parseFxExpr("DEFAULT_RTSIZE.x")), FxExprDependency::ResizeDependency), "dep resize");
    ok &= check(any(fxDependencies(parseFxExpr("VERTEXCOUNT+1")), FxExprDependency::ModelDependency), "dep model");
    ok &= check(any(fxDependencies(parseFxExpr("CLONEDVERTEXCOUNT")), FxExprDependency::ModelDependency),
                "dep cloned model");
    ok &=
        check(any(fxDependencies(parseFxExpr("TOTALMATERIAL")), FxExprDependency::MaterialDependency), "dep material");
    ok &= check(any(fxDependencies(parseFxExpr("CloneCount")), FxExprDependency::ModelDependency), "dep clone");
    ok &= check(any(fxDependencies(parseFxExpr("FRAME")), FxExprDependency::FrameDependency), "dep frame");
    {
        const auto mask = fxDependencies(parseFxExpr("DEFAULT_RTSIZE.x+VERTEXCOUNT"));
        ok &= check(any(mask, FxExprDependency::ResizeDependency) && any(mask, FxExprDependency::ModelDependency),
                    "dep combined");
    }
    ...
    // --- size resolver: ratio / rounding / inheritance / overflow ---
    {
        ...
        // Conv stage applies between expression evaluation and ratio.
        FxSizeExpr conv;
        conv.dimension = 2;
        conv.xExpr = "100";
        conv.yExpr = "100";
        conv.convX = 2.0F;
        conv.convY = 0.5F;
        const FxExtent converted = resolver.resolve(conv, ctx, table);
        ok &= check(converted.x == 200 && converted.y == 50, "size conv stage");

        // Depth ratio applies to the z axis of 3D extents.
        FxSizeExpr depthRatio;
        depthRatio.base = "WMap";
        depthRatio.xExpr = "";
        depthRatio.yExpr = "";
        depthRatio.zExpr = "16";
        depthRatio.depthRatio = 0.5F;
        const FxExtent halvedDepth = resolver.resolve(depthRatio, ctx, table);
        ok &= check(halvedDepth.z == 8 && halvedDepth.dimension == 3, "size depth ratio");
        ...
        // Per-effect transient budget guard (no allocation happens here).
        FxSizeResolver::checkTransientBytes(1024);
        ok &= expectThrows([&] { FxSizeResolver::checkTransientBytes(kMaxTransientBytesPerEffect + 1); },
                           "transient budget throws");
