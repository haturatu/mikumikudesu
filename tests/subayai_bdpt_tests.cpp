...
struct MockAccelerationBackend : dayo::graphics::IAccelerationBackend {
    ...
    createTlas(std::span<const dayo::graphics::AccelerationStructureHandle>) override {
        ++createTlasCalls;
        return next++;
    }
...
int main() {
    ...
    // DenoiserRuntime fallback: staging/readback -> CPU -> upload copy without CUDA.
    {
        ...
        // ensure() reports OIDN availability (false when OIDN is not linked);
        // extent acceptance is verified through the stored dimensions so this
        // stays green with and without OIDN.
        static_cast<void>(runtime.ensure(2, 1));
        ok &= check(runtime.width() == 2 && runtime.height() == 1, "denoiser ensure accepts extent");
