  ...
  private:
    ...
    FxExpr parseExpr(std::size_t depth) {
        ...
        for (;;) {
            ...
            out.node = FxBinaryExpr{c, std::move(lhsPtr), std::move(rhsPtr)};
            lhs = std::move(out);
        }
        leaveDepth();
        return lhs;
    ...
    FxExpr parseFactor(std::size_t depth) {
        ...
        if (c == '-' || c == '!') {
            ...
            trackNode();
            FxExpr out;
            out.node = FxUnaryExpr{c, std::make_shared<FxExpr>(std::move(operand))};
            return out;
        }
        ...
        try {
            if (!isDouble) {
                try {
                    ...
                    out.node = FxLiteralExpr{static_cast<std::int64_t>(value)};
                    return out;
            ...
            out.node = FxLiteralExpr{std::stod(tokenStr)};
    ...
    FxExpr parseIdentOrCall() {
        ...
        if (name == "true" || name == "false") {
            trackNode();
            FxExpr out;
            out.node = FxLiteralExpr{name == "true"};
            return out;
        }
        ...
        if (peek() == '(') {
            ...
            trackNode();
            FxExpr out;
            out.node = FxCallExpr{name, std::move(args)};
            return out;
        }
        pos_ = saved;
        trackNode();
        FxExpr out;
        out.node = FxIdentifierExpr{name};
        return out;
    }
...
FxScalar evalCall(const FxCallExpr& call, const FxEvalContext& context, FxCompatibilityProfile profile,
                  bool allowPowQuirk) {
    if (call.name == "pow") {
        if (call.args.size() != 2)
    ...
    if (call.name == "min" || call.name == "max") {
        ...
        // Evaluate each argument once: repeated evaluation would double
        // side effects on logging and waste work on deep subtrees.
        std::vector<FxScalar> values;
        values.reserve(call.args.size());
        bool hasDouble = false;
        for (const auto& arg : call.args) {
            values.push_back(evalNode(*arg, context, profile, allowPowQuirk));
            hasDouble = hasDouble || std::holds_alternative<double>(values.back());
        }
        if (hasDouble) {
            double best = toDoubleImpl(values[0]);
            for (std::size_t i = 1; i < values.size(); ++i) {
                const double v = toDoubleImpl(values[i]);
...
FxExprDependency depOfIdent(std::string_view name) noexcept {
    if (name == "DEFAULT_RTSIZE.x" || name == "DEFAULT_RTSIZE.y")
        return FxExprDependency::ResizeDependency;
    if (name == "VERTEXCOUNT" || name == "CLONEDVERTEXCOUNT" || name == "CloneCount" || name == "CLONECOUNT" ||
        name == "MODELINDEX")
        return FxExprDependency::ModelDependency;
    if (name == "TOTALMATERIAL")
        return FxExprDependency::MaterialDependency;
    if (name == "FRAME" || name == "FRAMEINDEX" || name == "SAMPLE" || name == "SAMPLEINDEX")
        return FxExprDependency::FrameDependency;
    // Unknown identifiers (including named resources, which decay through
    // the symbol resolver) carry no context-input dependency. Resource
    // invalidation is tracked via the resource table generation, not here.
    return FxExprDependency::StaticDependency;
}

FxExprDependency depOf(const FxExpr& expr) noexcept {
    if (std::holds_alternative<FxLiteralExpr>(expr.node))
        return FxExprDependency::StaticDependency;
    if (const auto* ident = std::get_if<FxIdentifierExpr>(&expr.node))
    ...
    if (const auto* binary = std::get_if<FxBinaryExpr>(&expr.node)) {
        FxExprDependency mask = FxExprDependency::StaticDependency;
