...
[[nodiscard]] std::int64_t resolveAxis(std::string_view exprText, std::int64_t inherited, float conv, float ratio,
    ...
    if (!exprText.empty()) {
        ...
        // finiteness + reject window below; keep double for conv/ratio first
        // so "conv -> ratio -> rounding" order matches the spec narrative
        // (integer truncation before conv/ratio would lose fractional scales).
    ...
    if (!std::isfinite(conv) || conv <= 0.0F)
        throw std::runtime_error("fx size conv must be positive finite: " + std::string(axis));
    if (!std::isfinite(ratio) || ratio <= 0.0F)
        throw std::runtime_error("fx size ratio must be positive finite: " + std::string(axis));
    const double scaled = raw * static_cast<double>(conv) * static_cast<double>(ratio);
...
void FxSizeResolver::checkTransientBytes(std::uint64_t bytes) {
    if (bytes > kMaxTransientBytesPerEffect)
        throw std::overflow_error("fx transient bytes per effect out of range");
}
...
FxExtent FxSizeResolver::resolve(const FxSizeExpr& expr, const FxEvalContext& context, const FxResourceTable& table,
    ...
    const std::int64_t x = resolveAxis(expr.xExpr, baseX, expr.convX, expr.widthRatio, expr.rounding, "x", context,
                                       table, profile, allowPowQuirk);
    std::int64_t y = 1;
    std::int64_t z = 1;
    if (dimension >= 2)
        y = resolveAxis(expr.yExpr, baseY, expr.convY, expr.heightRatio, expr.rounding, "y", context, table, profile,
                        allowPowQuirk);
    if (dimension >= 3)
        z = resolveAxis(expr.zExpr, baseZ, expr.convZ, expr.depthRatio, expr.rounding, "z", context, table, profile,
                        allowPowQuirk);
    ...
    if (dimension == 1) {
        ...
        out.x = static_cast<std::uint64_t>(x);
        out.y = 1;
        out.z = 1;
    } else {
        ...
        if (y <= 0 || static_cast<std::uint64_t>(y) > kMaxTextureDimension)
            throw std::overflow_error("fx texture height out of range");
        out.x = static_cast<std::uint64_t>(x);
        out.y = static_cast<std::uint64_t>(y);
        out.z = 1;
        if (dimension == 3) {
            if (z <= 0 || static_cast<std::uint64_t>(z) > kMaxTextureDimension)
                throw std::overflow_error("fx texture depth out of range");
            out.z = static_cast<std::uint64_t>(z);
        }
    }
