...
const std::string* MaterialBindingPlan::slotFor(std::string_view localId) const noexcept {
    try {
        auto findCanonical = [&](const std::string& key) -> const std::string* {
            const auto local = layout.localToCanonical.find(key);
        ...
        if (const auto* slot = findCanonical(std::string(localId)))
            return slot;
        return findCanonical(trimCopy(localId));
    } catch (...) {
        return nullptr;
    }
}
