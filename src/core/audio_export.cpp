...
void validateRequest(const AudioExportRequest& request) {
    if (request.source.empty()) throw std::invalid_argument("audio export source is empty");
    if (request.destination.empty()) throw std::invalid_argument("audio export destination is empty");
    if (request.bitrate == 0) throw std::invalid_argument("audio export bitrate must be positive");
    if (request.sampleRate == 0) throw std::invalid_argument("audio export sample rate must be positive");
...
void checkCancelled(const std::stop_token& stopToken) {
    if (stopToken.stop_requested()) throw std::runtime_error("audio export cancelled");
}
...
AudioExportResult exportM4aWithFfmpeg(
    const AudioExportRequest& request,
    const AudioExportProgressCallback& progress,
    ...
    try {
        ...
        const int audioStreamIndex = av_find_best_stream(
            context.input, AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
        if (audioStreamIndex < 0) throw std::runtime_error("audio export source has no audio stream");
        context.decoder = openDecoder(context.input, audioStreamIndex);
        const auto* inputStream = context.input->streams[audioStreamIndex];
        if (context.decoder->sample_rate <= 0) throw std::runtime_error("audio source has no sample rate");
        const int inputChannels = context.decoder->ch_layout.nb_channels;
        if (inputChannels <= 0) throw std::runtime_error("audio source has no channel layout");

        const auto* encoderCodec = avcodec_find_encoder(AV_CODEC_ID_AAC);
        if (encoderCodec == nullptr) throw std::runtime_error("AAC encoder is unavailable");
        context.encoder = avcodec_alloc_context3(encoderCodec);
        if (context.encoder == nullptr) throw std::bad_alloc();
        ...
        ffmpeg::check(avformat_alloc_output_context2(
            &context.output, nullptr, "ipod", request.destination.string().c_str()),
            "create M4A output");
        if (context.output == nullptr) throw std::runtime_error("create M4A output: unknown format");
        ...
        if (outputStream == nullptr) throw std::bad_alloc();
        ...
        ffmpeg::check(swr_alloc_set_opts2(
            &context.resampler, &context.encoder->ch_layout, context.encoder->sample_fmt,
            context.encoder->sample_rate, &inputLayout, context.decoder->sample_fmt,
            context.decoder->sample_rate, 0, nullptr), "create audio export resampler");
        av_channel_layout_uninit(&inputLayout);
        ffmpeg::check(swr_init(context.resampler), "initialize audio export resampler");
        context.fifo = av_audio_fifo_alloc(
            context.encoder->sample_fmt, static_cast<int>(request.channels), 4096);
        if (context.fifo == nullptr) throw std::bad_alloc();
        context.packet = av_packet_alloc();
        context.encodedPacket = av_packet_alloc();
        if (context.packet == nullptr || context.encodedPacket == nullptr) throw std::bad_alloc();

        const double inputTimeBase = av_q2d(inputStream->time_base);
        const double sourceDuration = inputStream->duration > 0
            ? static_cast<double>(inputStream->duration) * inputTimeBase
            : (context.input->duration > 0
                ? static_cast<double>(context.input->duration) / AV_TIME_BASE : 0.0);
        ...
        auto receivePackets = [&] {
            while (true) {
                ...
                ffmpeg::check(av_interleaved_write_frame(context.output, context.encodedPacket),
                              "write AAC packet");
        ...
        auto encodeSamples = [&](int sampleCount, bool pad) {
            ...
            try {
                ...
                ffmpeg::check(av_audio_fifo_read(
                    context.fifo, reinterpret_cast<void**>(frame->extended_data), sampleCount),
        ...
        auto encodeAvailable = [&](bool partial) {
            while (av_audio_fifo_size(context.fifo) >= frameSize
                   || (partial && av_audio_fifo_size(context.fifo) > 0)) {
        ...
        auto consumeDecodedFrame = [&](AVFrame* frame) {
            ...
            if (bytesPerSample <= 0 || frame->nb_samples <= 0) return;
            ...
            const double frameEnd = frameStart
                + static_cast<double>(frame->nb_samples) / context.decoder->sample_rate;
            ...
            const int skipSamples = std::clamp(
                static_cast<int>(std::floor((clipStart - frameStart) * context.decoder->sample_rate + 1e-6)),
                0, frame->nb_samples);
            const int clipSamples = std::clamp(
                static_cast<int>(std::ceil((clipEnd - (frameStart +
                    static_cast<double>(skipSamples) / context.decoder->sample_rate))
                    * context.decoder->sample_rate - 1e-6)),
                0, frame->nb_samples - skipSamples);
            if (clipSamples <= 0) return;

            std::vector<const std::uint8_t*> inputPointers(
                av_sample_fmt_is_planar(sampleFormat) ? inputChannels : 1);
            if (inputPointers.empty()) inputPointers.resize(1);
            if (av_sample_fmt_is_planar(sampleFormat)) {
                for (std::size_t channel = 0; channel < inputPointers.size(); ++channel) {
                    inputPointers[channel] = frame->extended_data[channel]
                        + static_cast<std::ptrdiff_t>(skipSamples * bytesPerSample);
                }
            } else {
                inputPointers[0] = frame->extended_data[0]
                    + static_cast<std::ptrdiff_t>(skipSamples * bytesPerSample
                                                   * inputChannels);
            }
            const auto outputCapacity = av_rescale_rnd(
                swr_get_delay(context.resampler, context.decoder->sample_rate) + clipSamples,
            ...
            if (converted == nullptr) throw std::bad_alloc();
            try {
                ...
                const int convertedSamples = swr_convert(
                    context.resampler, converted->extended_data, converted->nb_samples,
                    inputPointers.data(), clipSamples);
                ffmpeg::check(convertedSamples, "resample audio for export");
                if (convertedSamples > 0) {
                    ffmpeg::check(av_audio_fifo_realloc(
                        context.fifo, av_audio_fifo_size(context.fifo) + convertedSamples),
                                  "grow audio export buffer");
                    ffmpeg::check(av_audio_fifo_write(
                        context.fifo, reinterpret_cast<void**>(converted->extended_data), convertedSamples),
                ...
                currentProgress.processedSeconds = std::max(
                    currentProgress.processedSeconds,
                    std::min(totalSeconds, std::max(0.0, clipEnd - startSeconds)));
                if (progress) progress(currentProgress);
        ...
        if (decodedFrame == nullptr) throw std::bad_alloc();
        ...
        try {
            while (!reachedEnd) {
                ...
                if (context.packet->stream_index == audioStreamIndex) {
                    ffmpeg::check(avcodec_send_packet(context.decoder, context.packet),
                                  "submit audio export packet");
        ...
        while (true) {
            ...
            try {
                ...
                const int convertedSamples = swr_convert(
                    context.resampler, converted->extended_data, outputCapacity, nullptr, 0);
        ...
        if (selectedInputSamples == 0
            || (av_audio_fifo_size(context.fifo) == 0 && encodedSamples == 0)) {
...
#if DAYO_HAS_MEDIA
    if (!canExportM4a()) throw std::runtime_error("AAC encoder is unavailable");
