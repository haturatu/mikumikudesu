...
void sortKeys(std::vector<Key>& keys) {
...
template <typename Key>
void eraseIndices(std::vector<Key>& values, const std::vector<std::size_t>& indices) {
    std::size_t cursor = 0;
    values.erase(std::remove_if(values.begin(), values.end(), [&](const Key&) {
        return std::binary_search(indices.begin(), indices.end(), cursor++);
    }), values.end());
}
...
void appendSelected(std::vector<Key>& destination, const std::vector<Key>& source,
    ...
    for (const auto index : indices) if (index < source.size()) {
...
template <typename Key>
void pasteTrack(std::vector<Key>& destination, const std::vector<Key>& source,
                std::int64_t delta, MotionTrack track, std::vector<MotionKeyRef>& pasted) {
    for (auto key : source) {
...
void MotionEditor::erase(MotionDocument& document, std::vector<MotionKeyRef> keys) {
    ...
    for (const auto& key : keys) indices[static_cast<std::size_t>(key.track)].push_back(key.index);
...
void MotionEditor::move(MotionDocument& document, const std::vector<MotionKeyRef>& keys,
                        std::int64_t frameDelta) {
    for (const auto& key : keys) {
        switch (key.track) {
        case MotionTrack::bone: if (key.index < document.bones.size()) document.bones[key.index].frame = shiftedFrame(document.bones[key.index].frame, frameDelta); break;
        case MotionTrack::morph: if (key.index < document.morphs.size()) document.morphs[key.index].frame = shiftedFrame(document.morphs[key.index].frame, frameDelta); break;
        case MotionTrack::camera: if (key.index < document.cameras.size()) document.cameras[key.index].frame = shiftedFrame(document.cameras[key.index].frame, frameDelta); break;
        case MotionTrack::light: if (key.index < document.lights.size()) document.lights[key.index].frame = shiftedFrame(document.lights[key.index].frame, frameDelta); break;
        case MotionTrack::shadow: if (key.index < document.shadows.size()) document.shadows[key.index].frame = shiftedFrame(document.shadows[key.index].frame, frameDelta); break;
        case MotionTrack::ik: if (key.index < document.ik.size()) document.ik[key.index].frame = shiftedFrame(document.ik[key.index].frame, frameDelta); break;
        }
    }
    normalize(document);
}

MotionClipboard MotionEditor::copy(const MotionDocument& document,
    ...
    if (result.empty()) result.originFrame = 0;
    return result;
}

std::vector<MotionKeyRef> MotionEditor::paste(MotionDocument& document,
...
void EditMotionCommand::apply(Scene& scene) { static_cast<void>(scene.replaceMotion(after_, target_, global_)); }
void EditMotionCommand::undo(Scene& scene) { static_cast<void>(scene.replaceMotion(before_, target_, global_)); }
