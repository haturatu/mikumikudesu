...
const AVCodec* findVideoEncoder(VideoCodec codec) {
    switch (codec) {
    case VideoCodec::h264: return avcodec_find_encoder(AV_CODEC_ID_H264);
    case VideoCodec::h265: return avcodec_find_encoder(AV_CODEC_ID_HEVC);
    case VideoCodec::av1: return avcodec_find_encoder(AV_CODEC_ID_AV1);
    }
    return nullptr;
}
...
void freeFrame(AVFrame*& frame) noexcept { av_frame_free(&frame); }
void freePacket(AVPacket*& packet) noexcept { av_packet_free(&packet); }
...
struct VideoExporter::Impl {
    ...
    void initialize() {
        ...
        if (request.bitrate == 0) throw std::invalid_argument("video bitrate must be positive");
        if (request.includeAudio && request.audioBitrate == 0) {
            throw std::invalid_argument("audio bitrate must be positive");
        }
        if (request.destination.empty()) throw std::invalid_argument("video destination is empty");
        ...
        if (const auto parent = request.destination.parent_path(); !parent.empty()) {
            ...
            if (error) throw std::runtime_error("create video output directory: " + error.message());
        }
        partPath = request.destination;
        partPath += ".part";
        if (std::filesystem::exists(partPath)) {
            if (!request.overwrite) throw std::runtime_error("temporary output already exists: " + partPath.string());
            std::filesystem::remove(partPath);
        }

        const auto* encoder = findVideoEncoder(request.codec);
        if (encoder == nullptr) throw std::runtime_error("requested video encoder is unavailable");
        ffmpeg::check(avformat_alloc_output_context2(&format, nullptr, "mp4",
                                                     request.destination.string().c_str()),
                      "create MP4 output");
        try {
            videoStream = avformat_new_stream(format, encoder);
            if (videoStream == nullptr) throw std::bad_alloc();
            videoCodec = avcodec_alloc_context3(encoder);
            if (videoCodec == nullptr) throw std::bad_alloc();
            ...
            videoCodec->time_base = { frameRate.den > 0 ? frameRate.den : 1,
                                      frameRate.num > 0 ? frameRate.num : 30 };
            ...
            if (request.includeAudio) initializeAudio();
            scaler = sws_getContext(static_cast<int>(request.width), static_cast<int>(request.height),
                                    AV_PIX_FMT_RGBA, static_cast<int>(request.width),
                                    static_cast<int>(request.height), AV_PIX_FMT_YUV420P,
            ...
            ffmpeg::check(avio_open(&format->pb, partPath.string().c_str(), AVIO_FLAG_WRITE),
                          "open MP4 output");
    ...
    void initializeAudio() {
        ...
        if (encoder == nullptr) throw std::runtime_error("AAC encoder is unavailable");
        audioStream = avformat_new_stream(format, encoder);
        if (audioStream == nullptr) throw std::bad_alloc();
        audioCodec = avcodec_alloc_context3(encoder);
        if (audioCodec == nullptr) throw std::bad_alloc();
    ...
    void flushAudioResampler() {
        ...
        std::array<std::vector<float>, 2> converted {
            std::vector<float>(capacity), std::vector<float>(capacity)
        };
        while (true) {
            std::uint8_t* output[] {
    ...
    void encodePendingAudio(bool pad) {
        ...
        while (pendingAudio_[0].size() >= static_cast<std::size_t>(frameSize)
               || (pad && !pendingAudio_[0].empty())) {
            const auto count = pad
                ? std::min<std::size_t>(pendingAudio_[0].size(), static_cast<std::size_t>(frameSize))
    ...
    void close(bool commit) noexcept {
        if (format != nullptr && commit && headerWritten) av_write_trailer(format);
        ...
        if (videoCodec != nullptr) avcodec_free_context(&videoCodec);
        if (format != nullptr) avformat_free_context(format);
        format = nullptr;
...
void VideoExporter::writeVideoFrame(const ImageRgba8& image) {
#if DAYO_HAS_MEDIA
    if (impl_->finished) throw std::logic_error("video exporter is already finished");
    ...
    sws_scale(impl_->scaler, source, stride, 0, static_cast<int>(image.height),
              impl_->videoFrame->data, impl_->videoFrame->linesize);
...
void VideoExporter::writeAudio(std::span<const float> samples,
                               std::uint32_t sampleRate,
                               std::uint32_t channels) {
#if DAYO_HAS_MEDIA
    if (impl_->finished) throw std::logic_error("video exporter is already finished");
    if (!impl_->request.includeAudio || samples.empty()) return;
    ...
    const auto capacity = av_rescale_rnd(swr_get_delay(impl_->resampler, sampleRate)
                                         + static_cast<int64_t>(inputSamples),
    ...
    const int count = swr_convert(impl_->resampler, output, capacity, input,
                                  static_cast<int>(inputSamples));
...
VideoExportResult VideoExporter::finish() {
#if DAYO_HAS_MEDIA
    if (impl_->finished) throw std::logic_error("video exporter is already finished");
    if (impl_->videoFrames == 0) throw std::runtime_error("video export contains no frames");
    ...
    impl_->videoStream->duration = av_rescale_q(static_cast<std::int64_t>(impl_->videoFrames),
                                                impl_->videoCodec->time_base,
                                                impl_->videoStream->time_base);
    ...
    if (impl_->format->pb != nullptr) ffmpeg::check(avio_closep(&impl_->format->pb), "close MP4 output");
    impl_->finished = true;
    impl_->close(true);
    std::error_code error;
