...
AssetKind classifyAsset(const std::filesystem::path& path) {
    ...
    constexpr std::array<std::string_view, 7> images {
        ".bmp", ".jpg", ".jpeg", ".png", ".hdr", ".tga", ".dds"
    };
    ...
    if (contains(images, extension)) return AssetKind::image;
    if (contains(audio, extension)) return AssetKind::audio;
    if (contains(video, extension)) return AssetKind::video;
...
std::string_view toString(AssetKind kind) noexcept {
    ...
    case AssetKind::effect: return "effect";
