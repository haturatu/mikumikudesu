...
void writePixel(ImageRgba8& image, std::uint32_t x, std::uint32_t y,
...
std::uint8_t alphaIndex(const std::uint8_t* block, std::uint32_t pixel) {
    std::uint64_t bits = 0;
    for (std::uint32_t i = 0; i < 6; ++i) bits |= static_cast<std::uint64_t>(block[2 + i]) << (i * 8U);
...
ImageRgba8 decodeDds(const std::filesystem::path& path) {
    ...
    if (!input) throw std::runtime_error("cannot open DDS image: " + path.string());
    const auto end = input.tellg();
    if (end < 128) throw std::runtime_error("invalid DDS header: " + path.string());
    std::vector<std::uint8_t> bytes(static_cast<std::size_t>(end));
    input.seekg(0);
    input.read(reinterpret_cast<char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
    if (!input || std::memcmp(bytes.data(), "DDS ", 4) != 0 || u32(bytes.data() + 4) != 124U
        || u32(bytes.data() + 76) != 32U) throw std::runtime_error("invalid DDS file: " + path.string());
    ...
    if (code == fourCc('D', 'X', '1', '0')) {
        if (bytes.size() < 148) throw std::runtime_error("truncated DDS DX10 header");
        const auto dxgi = u32(bytes.data() + 128);
        dataOffset = 148;
        if (dxgi == 71 || dxgi == 72) code = fourCc('D', 'X', 'T', '1');
        else if (dxgi == 74 || dxgi == 75) code = fourCc('D', 'X', 'T', '3');
        else if (dxgi == 77 || dxgi == 78) code = fourCc('D', 'X', 'T', '5');
        else if (dxgi == 80 || dxgi == 81) code = fourCc('B', 'C', '4', 'U');
        else if (dxgi == 83 || dxgi == 84) code = fourCc('B', 'C', '5', 'U');
        else if (dxgi == 28 || dxgi == 29) code = fourCc('R', 'G', 'B', 'A');
        else if (dxgi == 87 || dxgi == 91) code = fourCc('B', 'G', 'R', 'A');
        else throw std::runtime_error("unsupported DDS DXGI format " + std::to_string(dxgi));
    }
    const auto payload = std::span<const std::uint8_t>(bytes).subspan(dataOffset);
    if (code == fourCc('D', 'X', 'T', '1')) return decodeBlocks(width, height, payload, BlockFormat::bc1);
    if (code == fourCc('D', 'X', 'T', '3')) return decodeBlocks(width, height, payload, BlockFormat::bc2);
    if (code == fourCc('D', 'X', 'T', '5')) return decodeBlocks(width, height, payload, BlockFormat::bc3);
    if (code == fourCc('A', 'T', 'I', '1') || code == fourCc('B', 'C', '4', 'U')) return decodeBlocks(width, height, payload, BlockFormat::bc4);
    if (code == fourCc('A', 'T', 'I', '2') || code == fourCc('B', 'C', '5', 'U')) return decodeBlocks(width, height, payload, BlockFormat::bc5);
    ...
    if (payload.size() < static_cast<std::size_t>(width) * height * 4U) throw std::runtime_error("truncated DDS pixels");
...
ImageRgba8 loadImageRgba8(const std::filesystem::path& path) {
    ...
    if (extension == ".dds") return decodeDds(path);
    int width = 0;
    int height = 0;
    int channels = 0;
