...
MediaFile::MediaFile(const std::filesystem::path& path) : impl_(std::make_unique<Impl>()) {
    ...
    if (!impl_->info.hasAudio && !impl_->info.hasVideo) throw std::runtime_error("media contains no audio or video stream");
...
const MediaInfo& MediaFile::info() const noexcept { return impl_->info; }
...
struct AudioPlayer::Impl { SDL_AudioStream* stream {}; };
