...
Float3 mul(const Float3& a, float b) {
...
Float3 normalized(const Float3& value) {
    const float l = length(value);
    return l > 1e-8F ? mul(value, 1.0F / l) : Float3{};
}
...
template <typename Key> struct KeyTrack {
...
template <typename Key> auto nextKey(KeyTrack<Key>& track, float frame) {
    if (frame < track.sampledFrame) {
        track.cursor = static_cast<std::size_t>(
            std::lower_bound(track.keys.begin(), track.keys.end(), frame,
                             [](const Key* key, float value) { return static_cast<float>(key->frame) < value; }) -
            track.keys.begin());
    } else {
        while (track.cursor < track.keys.size() && static_cast<float>(track.keys[track.cursor]->frame) < frame)
            ++track.cursor;
    }
...
struct BoneOrders {
    BoneOrder beforePhysics;
    BoneOrder afterPhysics;
};
...
LocalPose sampleBone(const std::vector<const VmdBoneKey*>& keys, float frame,
    ...
    if (keys.empty())
        return {};
    const auto next = std::lower_bound(keys.begin(), keys.end(), frame, [](const VmdBoneKey* key, float value) {
        return static_cast<float>(key->frame) < value;
    });
    if (next == keys.begin())
        return {(*next)->translation, normalize((*next)->rotation)};
    if (next == keys.end())
        return {keys.back()->translation, normalize(keys.back()->rotation)};
    ...
    const bool hasMethods = std::ranges::any_of(
        keys, [](const auto* key) { return std::ranges::any_of(key->methods, [](auto value) { return value != 0; }); });
    ...
    for (std::size_t axis = 0; axis < 3; ++axis) {
        ...
        result.translation[axis] =
            catmull(axis) ? catmullRom(previousControl->translation[axis], before->translation[axis],
    ...
    const float rt = bezier(linear, after->interpolation[3], after->interpolation[7], after->interpolation[11],
                            after->interpolation[15]);
    if (catmull(3)) {
        ...
        const auto align = [](Quat& value, const Quat& reference) {
            float product = 0.0F;
            for (std::size_t i = 0; i < 4; ++i)
                product += value[i] * reference[i];
            if (product < 0.0F)
                for (auto& component : value)
                    component = -component;
        };
        align(control0, control1);
        align(control2, control1);
        align(control3, control2);
        Quat interpolated{};
        for (std::size_t i = 0; i < 4; ++i) {
...
LocalPose sampleBone(BoneTrack& track, const std::unordered_map<std::uint32_t, BezierLut>& luts, float frame,
    ...
    const auto catmull = [&](std::size_t channel) {
        return track.hasMethods ? after->methods[channel] == 1 : fallbackMode == InterpolationMode::catmullRom;
    };
    LocalPose result;
    for (std::size_t axis = 0; axis < 3; ++axis) {
    ...
    if (catmull(3)) {
        Quat controls[4]{previousControl->rotation, before->rotation, after->rotation, nextControl->rotation};
        const auto align = [](Quat& value, const Quat& reference) {
            float product = 0.0F;
            for (std::size_t i = 0; i < 4; ++i)
                product += value[i] * reference[i];
        ...
        align(controls[0], controls[1]);
        align(controls[2], controls[1]);
        align(controls[3], controls[2]);
        Quat interpolated{};
        for (std::size_t i = 0; i < 4; ++i) {
            interpolated[i] = catmullRom(controls[0][i], controls[1][i], controls[2][i], controls[3][i], linear);
        }
        result.rotation = normalize(interpolated);
    } else {
        result.rotation = slerp(before->rotation, after->rotation, rt);
...
void calculateGlobalSubtree(const PmxModel& model, const std::vector<LocalPose>& local, std::vector<GlobalPose>& global,
...
const VmdIkKey* ikKeyAt(const VmdMotion* motion, float frame) {
    if (motion == nullptr || motion->ik.empty())
        return nullptr;
    const auto sorted =
        std::is_sorted(motion->ik.begin(), motion->ik.end(),
                       [](const VmdIkKey& left, const VmdIkKey& right) { return left.frame < right.frame; });
    if (sorted) {
        const auto key =
            std::upper_bound(motion->ik.begin(), motion->ik.end(), frame,
    ...
    for (const auto& key : motion->ik) {
        if (static_cast<float>(key.frame) <= frame && (result == nullptr || key.frame >= result->frame))
            result = std::addressof(key);
    }
    return result;
}
    ...
    for (const auto ikIndex : order) {
        ...
        for (int loop = 0; loop < loops; ++loop) {
            ...
            for (const auto& link : ik.ikLinks) {
                ...
                const auto toEffector =
                    normalized(sub(poses[effector].global.position, poses[linkIndex].global.position));
                ...
                const auto parentRotation = parent >= 0 && static_cast<std::size_t>(parent) < poses.size()
                                                ? poses[static_cast<std::size_t>(parent)].global.rotation
                ...
                poses[linkIndex].local.rotation =
                    multiply(poses[linkIndex].ikRotation, poses[linkIndex].withoutIk.rotation);
                poses[linkIndex].local.rotation = applyIkLimit(poses[linkIndex].local.rotation, link);
                poses[linkIndex].ikRotation =
...
void logLimbDiagnostics(const PmxModel& model, const std::unordered_map<std::string_view, BoneTrack>& boneTracks,
    ...
    if (static_cast<int>(frame) % 30 != 0)
        return;
    static constexpr std::array<std::string_view, 16> names{
        "左腕",   "左腕捩",   "左ひじ", "右腕",  "右腕捩", "右ひじ", "左足",   "左ひざ",
        "左足首", "左足ＩＫ", "左足IK", "左足D", "右足",   "右ひざ", "右足首", "右足ＩＫ",
    };
    for (const auto name : names) {
        ...
        log::debug("Bone sample: model=", model.metadata.modelName, ", frame=", frame, ", bone=", found->name,
                   ", vmdRot=(", sampled.rotation[0], ",", sampled.rotation[1], ",", sampled.rotation[2], ",",
                   sampled.rotation[3], ")", ", localRot=(", pose.rotation[0], ",", pose.rotation[1], ",",
                   pose.rotation[2], ",", pose.rotation[3], ")", ", globalPos=(", world.position[0], ",",
                   world.position[1], ",", world.position[2], ")");
    }
}
...
void skinQdef(PmxVertex& vertex, const PmxModel& model, const std::vector<GlobalPose>& global) {
    ...
    for (std::size_t influence = 0; influence < 4; ++influence) {
        ...
        const auto translation = sub(global[index].position, rotate(rotation, model.bones[index].position));
        auto dualPart = multiplyRaw({translation[0], translation[1], translation[2], 0.0F}, rotation);
        for (auto& component : dualPart)
            component *= 0.5F;
        if (!initialized) {
            pivot = rotation;
            initialized = true;
        } else if (dot({pivot[0], pivot[1], pivot[2]}, {rotation[0], rotation[1], rotation[2]}) +
                       pivot[3] * rotation[3] <
                   0.0F) {
            weight = -weight;
        }
    ...
    const float projection = real[0] * dual[0] + real[1] * dual[1] + real[2] * dual[2] + real[3] * dual[3];
    for (std::size_t component = 0; component < 4; ++component)
...
void MmdAnimator::setMotion(const VmdMotion* motion) {
    ...
    for (const auto& key : motion_->bones)
        impl_->boneTracks[key.name].keys.push_back(&key);
    for (const auto& key : motion_->morphs)
        impl_->morphTracks[key.name].keys.push_back(&key);
    for (auto& [name, track] : impl_->boneTracks) {
        std::ranges::sort(track.keys, {}, &VmdBoneKey::frame);
        track.hasMethods = std::ranges::any_of(track.keys, [](const auto* key) {
    ...
    for (auto& [name, track] : impl_->morphTracks)
        std::ranges::sort(track.keys, {}, &VmdMorphKey::frame);

    const auto compatibility = motionCompatibility();
    log::info("Motion compatibility: PMX bones=", compatibility.pmxBoneCount,
              ", VMD keys=", compatibility.vmdBoneKeyCount, ", VMD tracks=", compatibility.vmdBoneTrackCount,
...
MotionCompatibility MmdAnimator::motionCompatibility() const {
    ...
    for (const auto& key : motion_->bones) {
        ...
        if (pmxBones.contains(key.name))
            ++result.matchedBoneKeyCount;
    }
    result.vmdBoneTrackCount = vmdBones.size();
    for (const auto name : vmdBones) {
        if (pmxBones.contains(name))
            ++result.matchedBoneTrackCount;
    }
    return result;
}
...
AnimatedModelFrame MmdAnimator::evaluate(float frame, float deltaSeconds, bool gpuSkinning) {
    ...
    if (motion_ != nullptr) {
        if (const auto* key = ikKeyAt(motion_, frame); key != nullptr)
            result.visible = key->visible;
    }
    ...
    std::function<void(std::size_t, float)> applyMorph = [&](std::size_t index, float weight) {
        ...
        for (const auto& offset : morph.offsets) {
            if (morph.type == 0 || morph.type == 9)
                ...
                const auto last =
                    offset.index < 0 ? result.materials.size() : std::min(first + 1, result.materials.size());
                    ...
                    const auto apply4 = [&](Float4& value, const Float4& morphValue) {
                        for (std::size_t component = 0; component < 4; ++component) {
                            if (offset.operation == 0)
                                value[component] *= 1.0F + (morphValue[component] - 1.0F) * weight;
                            else
                                value[component] += morphValue[component] * weight;
                        }
                    };
    ...
    for (auto& vertex : result.vertices) {
        ...
        const std::size_t influenceCount =
            vertex.weightType == PmxWeightType::bdef1
                ? 1
        ...
        for (std::size_t influence = 0; influence < influenceCount; ++influence) {
            ...
            position = add(position,
                           mul(transformPoint(global[static_cast<std::size_t>(bone)],
                                              model_.bones[static_cast<std::size_t>(bone)].position, vertex.position),
                               weight));
