...
const std::unordered_map<std::string, std::string>& Localization::english() {
    static const std::unordered_map<std::string, std::string> table{
        ...
        {"window.external_parent", "External Parent"},
        {"window.material", "Material"},
        {"window.fx_debug", "FX Debug"},
...
const std::unordered_map<std::string, std::string>& Localization::japanese() {
    static const std::unordered_map<std::string, std::string> table{
        {"window.keyframe", "キーフレーム"}, {"window.bone", "ボーン"},
        {"window.interpolation", "補間"},    {"window.external_parent", "外部親"},
        {"window.material", "マテリアル"},   {"window.fx_debug", "FXデバッグ"},
        {"window.output", "出力"},           {"action.commit", "確定"},
        {"action.cancel", "キャンセル"},
    };
    return table;
}
