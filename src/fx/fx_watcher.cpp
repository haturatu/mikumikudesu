...
FxAssetWatcher::TimePoint FxAssetWatcher::mtimeOf(const std::filesystem::path& path) noexcept {
...
void FxAssetWatcher::addEffect(std::string effect, std::vector<std::filesystem::path> files) {
    ...
    for (auto& file : files) {
        ...
        if (!mtimes_.contains(id))
            mtimes_[id] = mtimeOf(file);
    }
...
void FxAssetWatcher::removeEffect(const std::string& effect) {
    ...
    for (auto it = mtimes_.begin(); it != mtimes_.end();) {
        if (!reverseDeps_.contains(it->first))
            it = mtimes_.erase(it);
        else
            ++it;
    }
...
std::vector<std::string> FxAssetWatcher::notifyChanged(const std::filesystem::path& file) {
    ...
    const auto id = key(file);
    const auto found = reverseDeps_.find(id);
...
std::vector<std::string> FxAssetWatcher::poll() {
    ...
    for (const auto& [id, previous] : mtimes_) {
        const auto current = mtimeOf(std::filesystem::path(id));
        if (current == previous)
            continue;
        mtimes_[id] = current;
        const auto found = reverseDeps_.find(id);
        if (found == reverseDeps_.end())
            continue;
        for (const auto& effect : found->second) {
            affected.insert(effect);
            dirty_.insert(effect);
        }
    }
...
void FxAssetWatcher::snapshot() {
    std::scoped_lock lock(mutex_);
    for (auto& [id, _] : mtimes_)
        mtimes_[id] = mtimeOf(std::filesystem::path(id));
}
